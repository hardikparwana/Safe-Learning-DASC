...
from robot_models.Unicycle2D_backup import *
...
class Actor:
    ...
    def policy(self,agent,target):
        ...
        const += [dh1_dxA @ agent.xdot(x) + dh1_dxB @ target.xdot(target.U) >= -alpha*h3]#np.linalg.norm(dh3_dxA @ (agent.g+ agent.g_corrected))/epsilon]
        # const += [alpha >= -20]
        ...
        try:
            ...
            return False, np.array([0,0]).reshape(-1,1), 0
        ...
        # print(solution)
        return True, solution.detach().numpy().reshape(-1,1), u_grad
...
def train(args):
    ...
    for ep in range(args.total_episodes):
        ...
        agentF = Unicycle2D_backup(np.array([0,0.2,0]),dt,3,FoV,max_D,min_D)
        ...
        # if 1:
            for st in range(args.max_steps):  # for each step, move 10 times??
                ...
                if 0:#and_sample<epsilon:
                    ...
                    solved, action, param_grad = agent.policy(agentF,agentT)
                    if solved==False:
                ...
                reward, reward_grad = agentF.compute_reward(agentT)
                ...
                FX_prev = FX
                t += dt
                # print("buffer",args.buffer_capacity)
                if (st+1) % args.horizon == 0 and args.train==True:
                    # print("st",st)
        ...
        return reward_episode, reward_moving_avg, alphas, ks
...
parser.add_argument('--max-steps', type=int, default=1500, metavar='N',help='maximum number of steps of each episode') #70
...
for Alpha in Alphas:
    for K in Ks:
        for Beta in Betas:
            ...
            episode_reward, moving_reward, alphas, ks =  train(args)
            axis1[0,0].plot(episode_reward,c = colors[index])
            axis1[1,0].plot(moving_reward,c = colors[index])
            axis1[0,1].plot(alphas,c = colors[index])
            axis1[1,1].plot(ks,c = colors[index])

            axis2[0].plot(episode_reward,c = colors[index])
            axis2[1].plot(alphas,c = colors[index],label='alpha')
            axis2[1].plot(ks,c = colors2[index],label='k')
