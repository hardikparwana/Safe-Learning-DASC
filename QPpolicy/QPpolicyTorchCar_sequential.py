...
class Actor:
    ...
    def policy(self,X,agent,horizon_step, t, c=0.3):
        ...
        # alpha1 = cp.Parameter(1,value = self.alpha1_tch.detach().numpy())
        # alpha2 = cp.Parameter(1,value = self.alpha2_tch.detach().numpy())
        ...
        try:
            ...
            print("self.alpha1_tch.grad",self.alpha1_tch.grad)
            print("alpha1_tch.grad",alpha1_tch.grad)
            print("alpha2_tch.grad",alpha2_tch.grad)

            # loss1 = h1_dot_ - self.alpha1_tch*h1_dot_ 
            # loss1.backward(retain_graph=True)            
            # grad1 = [self.alpha1_tch.grad.detach().numpy()[0]-self.alpha1_grad_sum, self.alpha2_tch.grad.detach().numpy()[0]-self.alpha2_grad_sum]
            ...
            # loss2 = h2_dot_ - self.alpha2_tch*h2_dot_ 
            # loss2.backward(retain_graph=True)
            # grad2 = [self.alpha1_tch.grad.detach().numpy()[0]-self.alpha1_grad_sum, self.alpha2_tch.grad.detach().numpy()[0]-self.alpha2_grad_sum]
    ...
    def updateParameterFeasibility(self,rewards,indexes):
        ...
        print("grad", len(self.index_tensor_grads))
        print(indexes[0])
        ...
        problem = cp.Problem(objective, const)
        problem.solve()
        print(f"direction: {d.value}, infeasible's:{d_infeasible.value}")
...
parser.add_argument('--horizon', type=int, default=100, metavar='N',help='RL time horizon') #3
...
parser.add_argument('--alpha2', type=float, default=2, metavar='G',help='CBF parameter')  #0.003
