...
def compute_barrier_differential(angle_diff,distance):
    ...
    if np.abs(angle_diff)>FoV:
        barrier_angle = -np.abs(FoV-np.abs(angle_diff))/FoV  #   -1
    else:
    ...
    if distance>max_D:
        ...
        mean_D = (max_D-min_D)/2
        c = mean_D**2
        barrier_distance = c - (distance - mean_D)**2
    # barrier_distance = (distance - min_D)*(max_D - distance)/(mean_D**2)
        
    barrier = barrier_angle*barrier_distance

    return barrier
    
def compute_reward(F_X,T_X,F_X_prev,T_X_prev):
    ...
    h2 = compute_barrier_differential(angle_diff,distance)
    ...
    h1 = compute_barrier_differential(angle_diff,distance)

    reward = h2 - h1 + alpha*h1    
...
def train(args):
    ...
    for ep in range(args.total_episodes):
        ...
        for st in range(args.max_steps):
            ...
            if (np.random.random()<epsilon):
                ...
                while not found:
                    ...
                    if compute_reward(F_ns_sim,T_ns_sim,F_ns_prev,T_ns_prev)>0:
                        found = True
        ...
        # if avg_reward > 290:
        if (ep % 100 == 0 and ep>1) or episodic_reward>70 :
            print("here 2")
            test_reward = eval_policy(agent, seed=88, eval_episodes=1)
            if test_reward > 300:
                print("here 3")
                final_test_reward = eval_policy(agent, seed=88, eval_episodes=1)
