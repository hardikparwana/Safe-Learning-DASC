...
class TD3:
    def __init__(
        ...
        # self.env = env
        self.upper_bound = float(5)#float(self.env.action_space.high[0]) #action space upper bound
        self.lower_bound = float(-5)#float(self.env.action_space.low[0])  #action space lower bound
        self.create_actor()
...
def train(args):
    ...
    for ep in range(args.total_episodes):
        ...
        for st in range(args.max_steps):
            ...
            rand_sample = np.random.random()
            if rand_sample<epsilon:
                d1 = -agent_TD3.upper_bound/2 + 2*agent_TD3.upper_bound/2*random.uniform(0,1)
                d2 = -agent_TD3.upper_bound/2 + 2*agent_TD3.upper_bound/2*random.uniform(0,1)
                d3 = -agent_TD3.upper_bound/2 + 2*agent_TD3.upper_bound/2*random.uniform(0,1)
            ...
            if (np.abs(np.sin(FX_prev[2,0]))>0.01):
                ...
                d2_obs = 0
            d3_obs =  wrap_angle(FX[2,0]-FX_prev[2])/dt - w
            ...
            # maximize reward
            reward = compute_reward_model_error( ( d1 - d1_obs ), ( d2 - d2_obs ), ( d3 - d3_obs ) )
            # print("reward: ",reward)
            ...
            # else:
            #     reward = 5 * reward
            agent_TD3.add_to_replay_memory(state, action, reward, next_state, done)          
        ...
        agent_TD3.learn_and_update_weights_by_replay(timestep)
        ...
        # print("6",epsilon)
        print('Ep. {}, Timestep {},  Ep.Timesteps {}, Episode Reward: {:.2f}, Moving Avg.Reward: {:.2f}, Fail Time: {:.2f} , epsilon: {:.2f}, Time: {}:{}:{}'
                .format(ep, total_timesteps, timestep,
                      episodic_reward[0], avg_reward, avg_timestep, epsilon, s//3600, s%3600//60, s%60)) 
